...
(function() {
    ...
    // 如果主应用没有暴露 dbManager，需要自己初始化
    // 但为了兼容，我们假设主应用已经初始化了 dbManager
    ...
    function showReminderModal(reminders, checkedKey) {
        // 获取主应用的样式变量
        const expenseColor = typeof uiSettings !== 'undefined' ? uiSettings.expenseColor : '#c2413a';
    ...
    async function openReminderNotebook() {
        ...
        if (typeof modalManager === 'undefined') {
            console.warn('modalManager 未定义，使用 alert 代替');
            alert('请从主应用打开提醒记事本');
            return;
        }
        
        const modal = modalManager.create('📝 提醒记事本', html, `<button id="closeReminderModal" class="close-btn">${typeof t !== 'undefined' ? t('close') : '关闭'}</button>`);
        if (!modal) return;
        ...
        modal.querySelectorAll('.reminder-toggle-btn').forEach(btn => {
            btn.onclick = async () => {
                ...
                if (updated) {
                    modalManager.close(modal);
                    openReminderNotebook();
                    showToast(updated.enabled ? '✅ 已启用' : '⏸️ 已停用');
                }
            };
        });
    ...
    function showTemplateSelector() {
        ...
        if (typeof uiSettings !== 'undefined' && uiSettings.darkMode) {
            const items = modal.querySelectorAll('[style*="color: #1e293b"]');
            items.forEach(el => {
                if (el.style.color === '#1e293b') el.style.color = '#e2e8f0';
            });
        }
        ...
        modal.querySelectorAll('.template-item').forEach(item => {
            item.addEventListener('mouseenter', () => {
                ...
                item.style.borderColor = 'var(--expense-color)';
            });
            ...
            item.onclick = async () => {
                ...
                const confirmModal = modalManager.create(
                    ...
                    `<div style="display:flex;gap:12px;">
                        <button id="confirmTemplateBtn" style="flex:1;background:var(--expense-color);color:white;border:none;padding:12px;border-radius:28px;">✅ 确认</button>
    ...
    function showAddReminderForm() {
        ...
        if (typeof uiSettings !== 'undefined' && uiSettings.darkMode) {
            ...
            const input = modal.querySelector('#reminderTextInput');
            if (input) {
                input.style.background = '#1e293b';
                input.style.color = '#e2e8f0';
            }
            const dateInput = modal.querySelector('#reminderSolarDate');
            if (dateInput) {
                dateInput.style.background = '#1e293b';
                dateInput.style.color = '#e2e8f0';
            }
        }
        ...
        modal.querySelectorAll('.date-type-btn').forEach(btn => {
            btn.onclick = () => {
                ...
                btn.style.borderColor = 'var(--expense-color)';
